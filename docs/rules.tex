...
\end{figure}
\newpage
\subsection*{Reverse pass ($\upuparrows$)}
\begin{figure}[!h] 
\centering
\fbox{
\begin{mathpar}
\inferrule* [right=RevLoop]
  ...
  (\ctxb{\Lambda_1'}{e_{body}}) \upuparrows (\ctxb{\Lambda_2}{\hat{e}_{body}}) \\
  ...
  \Lambda_3 = \Lambda_1 \cup \left(dom(\Lambda_{2,\Delta fv}) \mapsto \seq{\hat{fv}''}\right)
}
            {
                    ...
                    \upuparrows \left(\ctxb{\Lambda_3}{\Let{(\_~,\seq{\hat{rs}'}, \seq{\hat{fv}'})}{\hat{e}_{loop}}{(\Let{\seq{\hat{fv}''}}{(\Map~(+)~\seq{\hat{fv}'}~\seq{\hat{rs}'})}{\seq{\hat{fv}''}}})}\right)\\
            }

\inferrule* [right=RevIF]
            {
  \ctxb{\Lambda}{e_t} \upuparrows \ctxb{\Lambda_t}{\Let{\seq{\hat{fv}}_t}{\hat{e}_t}}{\seq{\hat{fv}}_t}\\
  \ctxb{\Lambda}{e_f} \upuparrows \ctxb{\Lambda_f}{\Let{\seq{\hat{fv}}_f}{\hat{e}_f}}{\seq{\hat{fv}}_f}\\
...
{
  \ctxb{\Lambda}{\Let{\seq{res}}{\If{e_p}{e_t}{e_f}}}{\seq{res}}\upuparrows \ctxb{\Lambda'}{\Let{\seq{\hat{res}}}{\If{e_p}{\hat{e}_t'}{\hat{e}_f'}}{\seq{\hat{res}}}}
}
\end{mathpar}
}
\end{figure}
